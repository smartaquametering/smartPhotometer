...
Version history
---------------
0.3 2026-08-02 Improve Description and Dokumentation.
...
//sysmode  = 2;																	// smart-Colorimeter-Case
	sys21    = "Warm-White-LED";									// Labeling back side
	sys22    = "Colorimeter";
...
//sysmode  = 4;																	// smart-IR-Turbidimeter-Case
...
// smart-LED-Array
// -------------------------------------------------------------------------------------------------------------------
//sysmode  = 21;																// smart-LED-Array Part 1 (LED Holder with Halogen Lamp)
//sysmode  = 22;																// smart-LED-Array Part 2 (Toslink Patch Panel with Halogen Lamp)
//sysmode  = 23;																// smart-LED-Array Part 3 (LWL Cabel Concentrator)
//sysmode  = 24;																// smart-LED-Array Part 4 (Pyramid Holder)

//sysmode  = 25;																// smart-LED-Array Part 5 (Pyramid sanding Guide)
...
// -------------------------------------------------------------------------------------------------------------------
casemode = 1;																	// smartAquaMetering-Case + Cuvette holder
...
// smart-LED-Array - Case dimensions
ledcx      = 148.0-2*ledcwo;										// Width inside                         [X]
...
module outercase(cib1,cib2,cib3,cif1,cif2,cif3,cil1,cil2,cil3,cir1,cir2,cir3){
	difference(){
		...
		outercaselabelings(cib1,cib2,cib3,cif1,cif2,cif3,cil1,cil2,cil3,cir1,cir2,cir3);
	}
	outercasesnapper();
}
...
// smartAquaMetering-Case
// - Outer Case Labelings
//
module outercaselabelings(cib1,cib2,cib3,cif1,cif2,cif3,cil1,cil2,cil3,cir1,cir2,cir3){
	// Back side
...
// smart-LED-Array Part 1 (LED Holder with Halogen Lamp)
module ledarraypart1(){
    difference(){
        // LED holder plate
        union (){
...
// smart-LED-Array Part 2 (Toslink Patch Panel with Halogen Lamp)
...
module ledarrayboardholders(ledhpx,ledcwo,ledcy,ledhbt,ledhx,ledhy,ledhz){
...
// smart-LED-Array Part 7 (LED Holder))
module ledarraypart7(){
	difference(){
		// LED holder plate
		union (){
			translate([0,0,0]) cube([ledbx,ledby,ledbz]);
		...
		translate([ledbx/9*8,ledby/6*3,0]) cylinder(d=ledbd,h=ledbz,$fn=cfn);
		// Row 4
		translate([ledbx/9*1,ledby/6*4,0]) cylinder(d=ledbd,h=ledbz,$fn=cfn);
		translate([ledbx/9*2,ledby/6*4,0]) cylinder(d=ledbd,h=ledbz,$fn=cfn);
		translate([ledbx/9*3,ledby/6*4,0]) cylinder(d=ledbd,h=ledbz,$fn=cfn);
		translate([ledbx/9*4,ledby/6*4,0]) cylinder(d=ledbd,h=ledbz,$fn=cfn);
		translate([ledbx/9*5,ledby/6*4,0]) cylinder(d=ledbd,h=ledbz,$fn=cfn);
		translate([ledbx/9*6,ledby/6*4,0]) cylinder(d=ledbd,h=ledbz,$fn=cfn);
		translate([ledbx/9*7,ledby/6*4,0]) cylinder(d=ledbd,h=ledbz,$fn=cfn);
...
// smart-LED-Array Part 8 (Case Lid)
module ledarraypart8(){
	// ledcwo = Case dimensions - Wall thickness outside
	// ledcx = Case dimensions - Width inside                         [X]
	// ledcy = Case dimensions - Length inside                        [Y]
...
for (i = [0:number-1]) {
	...
	if (sysmode==21) {
		// smart-LED-Array Part 1 (LED Holder with Halogen Lamp)
		translate([0,0,0]) ledarraypart1();
	}
	if (sysmode==22) {
		// smart-LED-Array Part 2 (Toslink Patch Panel with Halogen Lamp)
		translate([0,0,0]) ledarraypart2();
	}
	if (sysmode==23) {
		// smart-LED-Array Part 3 (LWL Cabel Concentrator)
		translate([0,0,0]) ledarraypart3();
	}
	if (sysmode==24) {
		// smart-LED-Array Part 4 (Pyramid Holder)
		translate([0,0,0]) ledarraypart4();
	}
	if (sysmode==25) {
		// smart-LED-Array Part 5 (Pyramid sanding Guide)
		translate([0,0,0]) ledarraypart5();
	}
	if (sysmode==26) {
		// smart-LED-Array Part 6 (Case)
		translate([0,0,0]) ledarraypart6();
	}
	if (sysmode==27) {
		// smart-LED-Array Part 7 (LED Holder)
		translate([0,0,0]) ledarraypart7();
	}
	if (sysmode==28) {
		// smart-LED-Array Part 8 (Case Lid)
		translate([0,0,0]) ledarraypart8();
	}
}
