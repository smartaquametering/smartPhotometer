...
//												SensorData[(s.Sensors[a].TaskValues[l].ValueNumber-1)]['MeasuredValue']=tempValue;
	...
	function UpdateSensorData() {
		...
		SensorData[6]['MeasuredValue'] = "<pre>Robertson<span style='white-space: pre'>: "+XYZ2CCT_Robertson(SensorXYZ)+"</br>McCamy   : "+XYZ2CCT_McCamy(SensorXYZ)+"</pre>";
		SensorData[7]['MeasuredValue'] = XYZ2DominantWavelength(SensorXYZ);
		SensorData[8]['MeasuredValue'] = IndexColors(IodineColorIndex)(ConvertXYZ(SensorXYZ), 2).map(item => item.name);
	...
	function UpdateReadingData() {
		...
		if (!isEmpty(EmptySampleXYZ)) {
			...
			ReadingData[10]['ColorSpaceValue01'] = "<pre>Robertson<span style='white-space: pre'>: "+XYZ2CCT_Robertson(EmptySampleXYZ)+"</br>McCamy   : "+XYZ2CCT_McCamy(EmptySampleXYZ)+"</pre>";
			ReadingData[11]['ColorSpaceValue01'] = XYZ2DominantWavelength(EmptySampleXYZ);
			ReadingData[12]['ColorSpaceValue01'] = IndexColors(IodineColorIndex)(ConvertXYZ(EmptySampleXYZ), 2).map(item => item.name);
		}
		if (!isEmpty(SampleXYZ)) {
			...
			ReadingData[10]['ColorSpaceValue1'] = "<pre>Robertson<span style='white-space: pre'>: "+XYZ2CCT_Robertson(SampleXYZ)+"</br>McCamy   : "+XYZ2CCT_McCamy(SampleXYZ)+"</pre>";
			ReadingData[11]['ColorSpaceValue1'] = XYZ2DominantWavelength(SampleXYZ);
			ReadingData[12]['ColorSpaceValue1'] = IndexColors(IodineColorIndex)(ConvertXYZ(SampleXYZ), 2).map(item => item.name);
		}
...
//			ReadingData[9]['ColorDifference']=printObject(Difference_RGB);
	...
	function DrawCharts() {
		...
		$('#CIExyChartBackground').css('background-image', "url('"+AssetsPath+"/images/cie.png')").css('background-repeat', 'no-repeat').css('background-position', boundingBox.left + "px " + boundingBox.top + "px").css('background-size', boundingBox.width + "px " + boundingBox.height + "px");
	}
